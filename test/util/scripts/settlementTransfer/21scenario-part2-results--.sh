...
echo "TODO => EXPECTED RESULT: As PS_TRANSFERS_RECORDED was sent for the second account another settlement transfer has been created."
...
echo "TODO => EXPECTED RESULT: Payee's settlement account has been also changed to PS_TRANSFERS_RECORDED. externalReference recorded."
echo
echo
...
echo "TODO => EXPECTED RESULT: As all 2 settlement accounts has been PS_TRANSFERS_RECORDED, entire settlment is PS_TRANSFERS_RECORDED."
...
echo "SKIP => EXPECTED RESULT: As the other settlement account is PS_TRANSFERS_RECORDED, another transfer has been prepared. Showing 4 records."
echo
echo

echo "TABLE transfer"
docker exec -it $DB_ID mysql -uroot -e "SELECT * FROM central_ledger.transfer ORDER BY createdDate DESC"
echo "SKIP => EXPECTED RESULT: New transfer record has been created. Showing 4 records."
echo
echo
...
echo "SKIP => EXPECTED RESULT: Only the regular transfers are fulfiled. No change here yet. Showing 2 records."
echo
echo
...
echo "TODO--similar to previous=> EXPECTED RESULT: Transfer participants are inserted for the prepared settlement transfer. 2 records inserted. Showing 8 records."
...
echo "SKIP => EXPECTED RESULT: Both regular transfers are COMMITTED. Both settlement transfers are RECEIVED_PREPARE."
