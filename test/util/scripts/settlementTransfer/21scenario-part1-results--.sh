...
echo "SKIP => EXPECTED RESULT: Two regular transfers for 600 and 200, and one settlement transfer for the amount of 800."
echo
echo

echo "TABLE settlement"
docker exec -it $DB_ID mysql -uroot -e "SELECT * FROM central_ledger.settlement ORDER BY createdDate DESC"
echo "X => EXPECTED RESULT: Settlement 1 is created."
echo
echo
...
echo "X => EXPECTED RESULT: Settlement state for settlementId 1 is PENDING_SETTLEMENT."
echo
echo
...
echo "X => EXPECTED RESULT: Settlement window 1 is CLOSED, window 2 is OPEN, and later window 1 is PENDING_SETTLEMENT."
echo
echo
...
echo "TODO => EXPECTED RESULT: Two settlement accounts are created, settlement transfer is created for the first one."
echo
echo
...
echo "TODO => EXPECTED RESULT: Both accounts are initially PENDING_SETTLEMENT, but later the first one is changed to PS_TRANSFERS_RECORDED."
echo
echo
...
echo "SKIP => EXPECTED RESULT: As one of the settlement accounts is PS_TRANSFERS_RECORDED, a transfer is prepared. Showing 3 records."
echo
echo
...
echo "SKIP => EXPECTED RESULT: Only the regular transfers are fulfiled. Showing 2 records."
echo
echo
...
echo "TODO => EXPECTED RESULT: Transfer participants are inserted for the prepared settlement transfer. Showing 6 records."
...
ORDER BY 1 DESC"
echo "SKIP => EXPECTED RESULT: The total SUM_amount for transferParticipant records should be always 0."
echo
echo
...
echo "TODO => EXPECTED RESULT: Both regular transfers are COMMITTED. The last record for the settlement transfer is RECEIVED_PREPARE."
...
ON p.participantId = pc.participantId
ORDER BY 1 DESC"
echo "SKIP => EXPECTED RESULT: Position is not affected by settlement being PS_TRANSFERS_RECORDED."
...
ORDER BY 1 DESC"
echo "SKIP => EXPECTED RESULT: Same as previous result, but showing history bottom-up. Showing 4 records."
echo
echo
